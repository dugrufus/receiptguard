      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 items-center">
          <div className="p-3 rounded-xl border bg-white">
            <div role="img" aria-label={`Barcode for order ${id}`} className="h-16 w-full rounded bg-white border overflow-hidden">
          ...
          <div className="p-3 rounded-xl border bg-white flex flex-col items-center">
            <div role="img" aria-label="QR code for competitor link" className="size-24 rounded border overflow-hidden" title="Competitor QR">
        ...
        <div className="flex gap-2">
          <button onClick={() => setOutcome("approved")} className="px-3 py-2 rounded-2xl border focus:outline-none focus:ring">{t("ish.outcome.approved")}</button>
          <button onClick={() => setOutcome("denied")}   className="px-3 py-2 rounded-2xl border focus:outline-none focus:ring">{t("ish.outcome.denied")}</button>
          <button onClick={() => setOutcome("later")}    className="px-3 py-2 rounded-2xl border focus:outline-none focus:ring">{t("ish.outcome.later")}</button>
        </div>

        {/* aria-live outcome note */}
        <div aria-live="polite" className="text-sm min-h-5">
